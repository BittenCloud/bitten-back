...
type Host struct {
	...
	Address       string                 `json:"address" gorm:"not null;uniqueIndex:idx_host_address_port_protocol"`                                               // Mandatory: The IP address or domain name of the host.
	Port          string                 `json:"port" gorm:"not null;uniqueIndex:idx_host_address_port_protocol"`                                                  // Mandatory: The port number for the host service.
	Protocol      string                 `json:"protocol" gorm:"type:varchar(10);not null;uniqueIndex:idx_host_address_port_protocol"`                             // Mandatory: The protocol (e.g., http, https, tcp).
	Network       string                 `json:"network,omitempty" gorm:"type:varchar(10);default:'tcp';index;uniqueIndex:idx_host_address_port_protocol_network"` // Network type (e.g., tcp, ws, grpc, kcp). Defaults to 'tcp'.
